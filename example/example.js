...
Elm.Example.make = function (_elm) {
   ...
   var aExpandIndex = $Signal.channel(-1);
         ...
         return function () {
            ...
            {case "_Tuple2":
               return A3(viewResult,
                 _U.eq(_v0._0,expandIndex),
                 _v0._0,
                 _v0._1);}
            _U.badCase($moduleName,
            "on line 44, column 36 to 76");
         }();
      },
      A3($List.map2,
      F2(function (v0,v1) {
      ...
      return function () {
         ...
         _L.fromArray([A2($Html.input,
                      _L.fromArray([$Html$Attributes.$class($Geocoding.isFailure(resp) ? "error" : "")
                                   ...
                                   $Signal.send(aLocation))]),
                      _L.fromArray([]))
                      ,resultHtml]));
      }();
   });
   var scene = F3(function (_v7,
   expandIndex,
   gc) {
      return function () {
         switch (_v7.ctor)
   ...
   scene,
   $Window.dimensions),
   $Signal.subscribe(aExpandIndex)),
   A2($Signal.merge,
   $Geocoding.byAddress(A2($Signal.map,
   function (addr) {
      return {_: {},address: addr};
   },
   A2(debounce,
   1 * $Time.second,
   $Signal.subscribe(aLocation)))),
   A2($Signal.sampleOn,
   $Signal.dropRepeats($Signal.subscribe(aLocation)),
...
Elm.Geocoding.make = function (_elm) {
      ...
      return function () {
         ...
         {case "_Tuple2":
            return A2($Basics._op["++"],
              ...
              "center=",
              A2($Basics._op["++"],
              $Basics.toString(coords.lat),
              A2($Basics._op["++"],
              ",",
              A2($Basics._op["++"],
              $Basics.toString(coords.lng),
              A2($Basics._op["++"],
              "&",
              A2($Basics._op["++"],
              "zoom=",
   ...
   var showLongitude = function (location) {
      return function () {
         ...
         "′ ",
         A2($Basics._op["++"],
         $Basics.toString($Basics.floor(s)),
         "″"))))));
      }();
   };
