...
Elm.Example.make = function (_elm) {
   ...
   if (_elm.Example.values)
   return _elm.Example.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $moduleName = "Example",
   $Basics = Elm.Basics.make(_elm),
   $Geocoding = Elm.Geocoding.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Graphics$Input = Elm.Graphics.Input.make(_elm),
   ...
   var debounce = F2(function (wait,
   signal) {
      return A2($Signal.sampleOn,
      $Signal.dropRepeats(A2($Time.since,
      wait,
      signal)),
      signal);
   });
   var aExpandIndex = $Graphics$Input.input(-1);
   var viewResult = F3(function (expand,
   index,
   gc) {
      return A2($Html$Tags.li,
      _L.fromArray([$Html$Attributes.$class(expand ? "expanded" : "")
                   ,A4($Html.on,
                   "click",
                   $Html.getAnything,
                   aExpandIndex.handle,
      ...
      _L.fromArray([A2($Html$Tags.div,
                   _L.fromArray([$Html$Attributes.$class("coords")]),
                   _L.fromArray([$Html.text($Geocoding.showLatitude(gc.geometry.location))
                                ,A2($Html$Tags.br,
                                _L.fromArray([]),
                                _L.fromArray([]))
                                ,$Html.text($Geocoding.showLongitude(gc.geometry.location))]))
                   ,A2($Html$Tags.img,
                   _L.fromArray([$Html$Attributes.src(expand ? A3($Geocoding.mapUrl,
                   gc.geometry.location,
                   A2($Geocoding.calcZoom,
                   gc.geometry.viewport,
                   600),
                   {ctor: "_Tuple2"
                   ,_0: 600
                   ,_1: 160}) : "")]),
   ...
   var viewResults = F2(function (expandIndex,
   gcs) {
      ...
      function (_v2) {
         return function () {
            switch (_v2.ctor)
            {case "_Tuple2":
               return A3(viewResult,
                 _U.eq(_v2._0,expandIndex),
                 _v2._0,
                 _v2._1);}
            _E.Case($moduleName,
            "on line 42, column 31 to 71");
         }();
      },
      A2($List.zip,
      _L.range(1,$List.length(gcs)),
      gcs)));
   });
   var aLocation = $Graphics$Input.input("");
   var view = F2(function (expandIndex,
   resp) {
      return function () {
         var resultHtml = function () {
            ...
            {case "Failure":
               return A2($Html$Tags.span,
                 ...
                 _L.fromArray([$Html.text($Geocoding.showStatus(resp._0))]));
               case "Success":
               return A2(viewResults,
                 expandIndex,
                 resp._0);
               case "Waiting":
               return A2($Html$Tags.div,
                 _L.fromArray([$Html$Attributes.$class("waiting")]),
                 _L.fromArray([]));}
            _E.Case($moduleName,
         ...
         _L.fromArray([A2($Html$Tags.input,
                      _L.fromArray([$Html$Attributes.$class($Geocoding.isFailure(resp) ? "error" : "")
                                   ,$Html$Attributes.type$("text")
                                   ,$Html$Attributes.placeholder("Enter address or location...")
                                   ,A4($Html.on,
                                   "keyup",
                                   $Html.getValue,
                                   aLocation.handle,
      ...
      return function () {
         ...
         {case "_Tuple2":
            return A3($Html.toElement,
              _v9._0,
              _v9._1,
              A2(view,expandIndex,gc));}
         _E.Case($moduleName,
         "on line 65, column 31 to 65");
      }();
   });
   ...
   aExpandIndex.signal),
   A2($Signal.merge,
   $Geocoding.byAddress(A2($Signal.lift,
   function (addr) {
      return {_: {},address: addr};
   },
   A2(debounce,
   1 * $Time.second,
   aLocation.signal))),
   A2($Signal.sampleOn,
   $Signal.dropRepeats(aLocation.signal),
   $Signal.constant($Geocoding.Waiting))));
   _elm.Example.values = {_op: _op
                         ,aLocation: aLocation
                         ,aExpandIndex: aExpandIndex
                         ...
                         ,debounce: debounce};
   return _elm.Example.values;
};Elm.Geocoding = Elm.Geocoding || {};
Elm.Geocoding.make = function (_elm) {
   "use strict";
   _elm.Geocoding = _elm.Geocoding || {};
   if (_elm.Geocoding.values)
   return _elm.Geocoding.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$Geocoding = Elm.Native.Geocoding.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var calcZoom = F2(function (viewport,
   displayWidth) {
      return function () {
         var angle = viewport.northeast.lng - viewport.southwest.lng;
         var adjAngle = _U.cmp(angle,
         0) < 0 ? angle + 360 : angle;
         var globeWidth = 256.0;
         return $Basics.round(A2($Basics.logBase,
         2,
         $Basics.toFloat(displayWidth) * 360 / angle / globeWidth));
      }();
   });
   var mapUrl = F3(function (coords,
   zoom,
   _v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return _L.append("https://maps.googleapis.com/maps/api/staticmap?",
              _L.append("center=",
              _L.append($String.show(coords.lat),
              _L.append(",",
              _L.append($String.show(coords.lng),
              _L.append("&",
              _L.append("zoom=",
              _L.append($String.show(zoom),
              _L.append("&",
              _L.append("size=",
              _L.append($String.show(_v0._0),
              _L.append("x",
              $String.show(_v0._1)))))))))))));}
         _E.Case($moduleName,
   ...
   var byAddress = $Native$Geocoding.byAddress;
   var isFailure = function (gr) {
      return function () {
         switch (gr.ctor)
   ...
   var showLongitude = function (location) {
      return function () {
         var d = $Basics.abs(location.lng);
         var m = (d - $Basics.toFloat($Basics.floor(d))) * 60;
         var s = (m - $Basics.toFloat($Basics.floor(m))) * 60;
         var ns = _U.cmp(location.lng,
         0) > 0 ? "E " : "W ";
         return _L.append(ns,
         _L.append($String.show($Basics.floor(d)),
         _L.append("° ",
         _L.append($String.show($Basics.floor(m)),
         _L.append("′ ",
         _L.append($String.show($Basics.floor(s)),
         "″"))))));
      }();
   };
   var showLatitude = function (location) {
      return function () {
         var d = $Basics.abs(location.lat);
         var m = (d - $Basics.toFloat($Basics.floor(d))) * 60;
         var s = (m - $Basics.toFloat($Basics.floor(m))) * 60;
         var ns = _U.cmp(location.lat,
         0) > 0 ? "N " : "S ";
         return _L.append(ns,
         _L.append($String.show($Basics.floor(d)),
         _L.append("° ",
   ...
   var InvalidRequest = {ctor: "InvalidRequest"};
   var RequestDenied = {ctor: "RequestDenied"};
   var OverQueryLimit = {ctor: "OverQueryLimit"};
   var ZeroResults = {ctor: "ZeroResults"};
   var OK = {ctor: "OK"};
   ...
   var GeocodeRequest = function (a) {
      return {_: {},address: a};
   };
   var Geocode = F5(function (a,
   b,
   c,
   d,
   e) {
   ...
   var Address = F4(function (a,
   b,
   c,
   d) {
      return {_: {}
             ,long_name: b
             ,postcode_localities: c
             ,short_name: a
             ,types: d};
   });
   var Geometry = F3(function (a,
   b,
   c) {
      return {_: {}
             ,bounds: c
